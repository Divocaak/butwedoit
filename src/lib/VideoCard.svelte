...
>
	...
	>
		<div class="card-content-holder-small text-center px-2">
			<h2 class="unbounded pt-1 mb-1 lh-sm" style="font-size: .6em;">{label}</h2>
			<p class="lead roboto pb-1 lh-sm" style="font-size: .6em;">{shortDesc}</p>
		</div>
	</div>
</div>
...
<style>
	...
	.card-background-effect:hover {
		transform: scale(1.1);
		filter: saturate(1);
	}

	.card-background-effect-wrapper {
		max-width: 100%;
		max-height: 100%;
		overflow: hidden;
	}
